...
Mainsail provides a couple components to aid in providing easy access to animating components.
...
-   Allows for animation lifecycle callbacks
...
// The hook provides all state handling necessary for using Switcher
const {
    previousView,
    currentView,
    previousAnim,
    ...
    // Just providing some defaults initial config
    currentView: 1,
...
After the setup above, `setView()` is the only method needed to change views.
