...
<Meta title="React/Guides/AutoGrid" />

# INTRODUCING: The `<AutoGrid/>` Component
...
> WARNING: the `colEnd` can be a little tricky in that you are not ending the span with the column containing the content, you're ending it _before_ the column number specified.
